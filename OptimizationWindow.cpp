	...
	double *InitialValue, double *MinBound, double *MaxBound, int InitialType, int CallbackInterval)
{
	...
	for(int Par = 0; Par < NPars; ++Par)
	{
		String Filename0 = Format("TestFile%d.txt", Par);
		std::string Filename = Filename0.ToStd();
		std::ofstream TestFile(Filename.data());
		
		for(int Step = 0; Step < NSteps; ++Step)
		{
			for(int Walker = 0; Walker < NWalkers; ++Walker)
			{
				TestFile << "\t" << Data(Walker, Par, Step);
			}
			TestFile << "\n";
		}
		/*
		...
		for(int Step = 0; Step < NSteps; ++Step)
		{
			for(int Walker = 0; Walker < NWalkers; ++Walker)
			{
				TestFile << "\t" << Data.LLValue(Walker, Step);
			}
			TestFile << "\n";
		}
		*/
			...
			if(Data->NPars > 1)
			{
				for(int Par1 = 0; Par1 < Data->NPars; ++Par1)
				{
					for(int Par2 = Par1+1; Par2 < Data->NPars; ++Par2)
					{
						...
						double StrideX = (Maxx - Minx)/(double)(DistrResolution+1);
						double StrideY = (Maxy - Miny)/(double)(DistrResolution+1);
...
void MCMCResultWindow::BeginNewPlots(mcmc_data *Data, double *MinBound, double *MaxBound, const Array<String> &FreeSyms)
{
	...
	if(Data->NPars > 1)
	{
		...
		int Dim            = 60; //Size of the plot area excluding margins
		int SmallmarginY   = 5;
		int SmallmarginX   = 15;
		int Largemargin    = 40;
		int DimX = 0;
		...
		for(int Par1 = 0; Par1 < Data->NPars; ++Par1)
		{
			...
			for(int Par2 = Par1+1; Par2 < Data->NPars; ++Par2)
			{
				...
				//TODO: We technically only need one X and Y axis per parameter, not per pair
				//of parameters...
